class BloodProbe < Struct.new(:blood_test)
  ...
  HEMOGLOBIN_TEST= lambda{|test| HEMOGLOBIN_RANGE.include? test.hb}
  MEAN_CELL_VOLUME_TEST = lambda{|test| MEAN_CELL_VOLUME_RANGE.include? test.mcv }
  WHITE_BLOOD_CELLS_TEST = lambda{|test| WHITE_BLOOD_CELLS_RANGE.include? test.wbc }
  PLATELETS_TEST = lambda{|test| PLATELETS_RANGE.include? test.platelets }
  NEUTROPHILS_TEST = lambda{|test| NEUTROPHILS_RANGE.include? test.neutrophils }
  LYMPHOCYTES_TEST = lambda{|test| LYMPHOCYTES_RANGE.include? test.lymphocytes }
  ALANINE_AMINOTRANSFERASE_TEST = lambda{|test| ALANINE_AMINOTRANSFERASE_RANGE.include? test.alt }
  ALKALINE_PHOSPHATASE_TEST = lambda{|test| ALKALINE_PHOSPHATASE_RANGE.include? test.alk_phos }
  CREATININE_TEST = lambda{|test| CREATININE_RANGE.include? test.creatinine }
  ERYTHROCYTE_SEDIMENTATION_RATE_TEST = lambda{|test| ERYTHROCYTE_SEDIMENTATION_RATE_RANGE.include? test.esr }
  C_REACTIVE_PROTEIN_TEST = lambda{|test| C_REACTIVE_PROTEIN_RANGE.include? test.crp }
